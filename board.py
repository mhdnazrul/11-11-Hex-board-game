...
class HexBoard:
    ...
    def is_connected(self, player, debug=False):
        ...
        if player == 1:
            # Seed from a virtual column on the RIGHT edge (col = size).
            # Include ALL rows (range(self.size)) so the bottom-right corner
            # cell (size-1, size-1) is reachable directly, not only via
            # adjacency from its neighbour. Previously range(self.size-1)
            # missed the last row, which could cause incorrect win detection.
            searchq = [(self.size, i, self.size) for i in range(self.size)]
        else:
            # Seed from a virtual row on the BOTTOM edge (row = size).
            searchq = [(self.size, self.size, i) for i in range(self.size)]
